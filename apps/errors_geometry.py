...
@app.cell
def _():
    df_true = get_table_view(lambda row: {"energy_PBE [eV/atom]": row.uncorrected_energy/ row.natoms, "formula":row.formula})
    return (df_true,)
...
def _(df_deltas):
    ...
    ax = sns.boxplot(df_deltas, x="MAE_XYZ", y="uMLIPs", hue="id_dimension", **dict(
...
@app.cell
def _(df_deltas, dropdow_elements, dropdow_models):
